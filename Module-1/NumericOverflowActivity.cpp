...
bool add_numbers(T const& start, T const& increment, unsigned long int const& steps, T& result)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        ...
        // Check for potential underflow if increment is negative
        if ((increment < 0) && (result < std::numeric_limits<T>::min() - increment))
            return false; // underflow (for signed types)
        result += increment;
    }
    return true;
}
...
bool subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps, T& result)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        ...
        // Check for potential overflow if decrement is negative
...
void test_overflow()
{
    ...
    // Check return status of add_numbers() to report overflow
...
void test_underflow()
{
    ...
    // Check return status of subtract_numbers() to report underflow
