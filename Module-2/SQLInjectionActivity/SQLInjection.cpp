...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Convert SQL to lowercase for pattern matching
  std::string lowered_sql = sql;
  std::transform(lowered_sql.begin(), lowered_sql.end(), lowered_sql.begin(), ::tolower);

  // Extract portion of SQL starting from WHERE clause
  size_t where_pos = lowered_sql.find("where");
  if (where_pos != std::string::npos)
  {
      std::string where_clause = lowered_sql.substr(where_pos);

      // Detect suspicious "OR value=value" patterns
      if (where_clause.find(" or ") != std::string::npos && where_clause.find("=") != std::string::npos)
      {
          std::cout << "ERROR: Potential SQL Injection detected and blocked." << std::endl;
          std::cout << "Blocked Query: " << sql << std::endl;
          return false;
      }
  }
  ...
  {
      std::cout << "Query execution failed. ERROR = " << error_message << std::endl;
      sqlite3_free(error_message);
      return false;
  }
